...
//
// Auth: the company presents its registry token (stored per casting tenant). ONE
// slug — the casting tenant slug IS the HoldCrew company slug. The caller passes
// the slug from the authenticated request (never client input), so a booking can
// only ever write to that company's own Job Log.
...
async function promoteToJobLog(slug, job, talent) {
  ...
  const payload = { company: slug, token, tab: jobTab(job), ...talent };

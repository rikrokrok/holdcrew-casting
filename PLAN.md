...
- **Book → promote:** when a candidate is marked **Booked**, casting calls a HoldCrew **n8n webhook**
  (v3-sync-talent lineage) to write/patch a **TALENT row in that job's Job Log** (Department=Talent).
  This is the single cross-system write; casting owns curation, HoldCrew owns the job record. Usage/
  buyout/fees captured at Book ride along and feed HoldCrew's existing **Talent** feature
  ("session & usage costs").
...
8. **Book → promote** — n8n webhook writes the Job Log TALENT row; capture usage/buyout/fees.
